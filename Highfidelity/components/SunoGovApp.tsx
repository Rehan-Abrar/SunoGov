...
// Shared spring configs - smoother and more fluid
...
function HomePage({ navigate }: { navigate: (p: Page) => void }) {
  ...
  const handleSubmit = () => {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); navigate("results"); }, 1100);
  };
        ...
        <div className="rounded-2xl sm:rounded-3xl bg-card border border-border shadow-sm transition-shadow focus-within:shadow-md">
          ...
          <div className="flex items-center gap-2.5 sm:gap-2 px-4 sm:px-3 py-3.5 sm:py-3">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <motion.button
                initial="rest"
                whileHover={!text.trim() || loading ? "rest" : "hover"}
                whileTap={{ scale: 0.94 }}
                transition={SPRING_SNAPPY}
...
const COMPLAINT_EN = `To,
The Executive Director
Water and Sanitation Agency (WASA)
Lahore, Punjab

Subject: Broken Water Pipe Causing Road Damage — Urgent Action Required

Respected Sir/Madam,

I am writing to report a broken water pipe on Main Boulevard, Gulberg, Lahore. The pipe has been leaking for approximately three days, causing significant road damage and creating a hazardous condition for pedestrians and vehicles alike.

The leak is located near the junction of Main Boulevard and Jail Road. The resulting water accumulation has weakened the road surface, and a visible pothole has formed at the site.

I request that your department urgently dispatch a repair team to address this issue. Prompt action will prevent further road deterioration and ensure public safety.
...
const COMPLAINT_UR = `بخدمت،
ایگزیکٹو ڈائریکٹر
واٹر اینڈ سینیٹیشن ایجنسی (واسا)
لاہور، پنجاب

موضوع: ٹوٹا ہوا پانی کا پائپ جو سڑک کو نقصان پہنچا رہا ہے — فوری توجہ درکار

جناب/محترمہ،

میں آپ کو مین بلیوارڈ، گلبرگ، لاہور میں ٹوٹے ہوئے پانی کے پائپ کے بارے میں مطلع کرنا چاہتا/چاہتی ہوں۔ یہ پائپ تقریباً تین دنوں سے رس رہا ہے، جس سے سڑک کو نمایاں نقصان پہنچ رہا ہے اور پیدل چلنے والوں اور گاڑیوں کے لیے خطرناک صورتحال پیدا ہو رہی ہے۔

یہ رساو مین بلیوارڈ اور جیل روڈ کے جنکشن کے قریب ہے۔ پانی کے جمع ہونے سے سڑک کی سطح کمزور ہو گئی ہے اور موقع پر ایک واضح گڑھا بن گیا ہے۔

میں درخواست کرتا/کرتی ہوں کہ آپ کا محکمہ فوری طور پر ایک مرمتی ٹیم بھیجے۔
...
const CHANNELS = [
  {
    ...
    icon: Globe,
    label: "Online Portal",
    value: "wasa.punjab.gov.pk/complaint",
    action: "open",
  ...
  },
  {
    ...
    icon: EnvelopeSimple,
    label: "Email",
    value: "complaints@wasa.punjab.gov.pk",
    action: "email",
    href: "mailto:complaints@wasa.punjab.gov.pk",
  },
  {
    type: "APP",
    icon: DeviceMobileCamera,
    label: "Mobile App",
    value: "Punjab Saaf Pani App",
    action: "download",
    href: "#",
  },
  {
  ...
  },
  {
    ...
    icon: Clock,
    label: "Office Hours",
    value: "Mon to Fri, 9 am to 5 pm",
    action: null,
    href: null,
  },
];

function ResultsPage({ navigate }: { navigate: (p: Page) => void }) {
  const containerRef = useRef<HTMLDivElement>(null);
  const { lang } = useLang();
  const t = T[lang];
          ...
          <div className="rounded-3xl sm:rounded-3xl bg-card border border-border overflow-hidden">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <h2 className="text-[17px] sm:text-[17px] md:text-[18px] font-semibold text-foreground leading-snug">
                    Broken water pipe causing road damage
                  </h2>
            ...
            <div className="px-6 sm:px-6 py-5 sm:py-5 border-t border-border bg-muted/30">
              <p className="text-[14px] sm:text-[13.5px] text-muted-foreground leading-relaxed italic">
                Water supply and sewerage infrastructure falls under WASA jurisdiction. Road damage caused by water leaks is a secondary issue handled by the same authority.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="sg-result-section">
              ...
              {(["en", "ur"] as const).map((lang) => (
                <motion.button
                  ...
                  className={`relative px-3.5 sm:px-2.5 h-8 sm:h-6 rounded-lg sm:rounded-md text-[13px] sm:text-[11px] font-medium focus-visible:outline-none ${
                    lang === "ur" ? "font-['Noto_Nastaliq_Urdu',serif]" : ""
                  } ${
                    activeTab === lang
                      ? "text-foreground"
                ...
                >
                  ...
                  <span className="relative z-10">{lang === "en" ? "English" : "اردو"}</span>
                </motion.button>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-3xl sm:rounded-3xl bg-card border border-border overflow-hidden">
            <div className="flex items-center justify-between px-5 sm:px-5 py-3.5 sm:py-2.5 border-b border-border">
              ...
              <CopyButton text={activeTab === "en" ? COMPLAINT_EN : COMPLAINT_UR} />
            </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                >
                  {activeTab === "en" ? COMPLAINT_EN : COMPLAINT_UR}
                </pre>
              </motion.div>
            </AnimatePresence>
            ...
            <div className="px-5 sm:px-5 py-4 sm:py-3 border-t border-border">
              <motion.button
                whileTap={{ scale: 0.98 }}
                onClick={() => {
                  const content = activeTab === "en" ? COMPLAINT_EN : COMPLAINT_UR;
